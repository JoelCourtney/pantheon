...
impl TryFrom<PathBuf> for CharacterFile {
    ...
    fn try_from(path: PathBuf) -> Result<Self, Self::Error> {
        ...
        let second_dot = file_name[first_dot + 1..].find('.').ok_or(NoSecondDot)? + first_dot + 1;
        Ok(CharacterFile {
...
pub enum CharacterFileError {
    #[error("couldn't convert full path to str")]
    FullPath,
    #[error("path didn't have a file name")]
    NoFileName,
    #[error("couldn't convert OsStr to str")]
    OsString,
    #[error("file name wasn't in full path")]
    CouldntFindFileName,
    #[error("file name has no dots")]
    NoFirstDot,
    #[error("file name has only one dot")]
    NoSecondDot,
}
